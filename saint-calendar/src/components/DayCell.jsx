import { FEAST_TYPES, LITURGICAL_COLORS } from '../data/feasts'

/** Left-edge colour bar indicating liturgical colour */
function ColourBar({ liturgicalColor }) {
  const col = LITURGICAL_COLORS[liturgicalColor]
  if (!col) return null
  return (
    <span
      className={`absolute left-0 top-0 bottom-0 w-1 rounded-l-md ${col.indicator}`}
    />
  )
}
...
export default function DayCell({ day, feasts, country, isToday, isCurrentMonth, onFeastClick }) {
  if (!isCurrentMonth) {
    return <div className="min-h-[90px] bg-stone-50/40 rounded-lg p-1" />
  }
      ...
      <div className="flex items-center justify-between mb-1">
        <span
          className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-sm font-semibold
            ${isToday ? 'bg-yellow-600 text-white shadow-sm' : 'text-stone-700'}`}
        >
          {day}
        </span>
      </div>
      ...
      <div className="space-y-0.5">
        {visibleFeasts.map(feast => {
          ...
          return (
            <button
              ...
              className={`relative w-full text-left rounded-md pl-3 pr-1.5 py-0.5 text-xs leading-tight transition-all
                hover:scale-[1.02] hover:shadow-sm active:scale-100 overflow-hidden
                ${isHolyForCountry
            ...
            >
              {/* Liturgical colour left-edge bar */}
              <ColourBar liturgicalColor={feast.liturgicalColor} />
        ...
        {overflow > 0 && (
          <button
            ...
            className="w-full text-left text-xs text-stone-400 hover:text-stone-600 pl-1 font-medium transition-colors"
          >
            +{overflow} more…
          </button>
        )}
